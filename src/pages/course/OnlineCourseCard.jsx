  ...
  useEffect(() => {
    ...
    const loadCourses = async () => {
      try {
        ...
        // Filter for online courses
        const onlineCourses = (data || []).filter((c) => {
          const courseType = ((c.course_type || c.type || "") + "").trim().toLowerCase();
          return courseType.includes("online");
        });

        console.log("📦 Fetched online courses:", onlineCourses.length);
        if (onlineCourses.length > 0) {
          console.log("💰 Sample course pricing:", {
            title: onlineCourses[0].title,
            price_cents: onlineCourses[0].price_cents,
            price: onlineCourses[0].price,
            offerPrice: onlineCourses[0].offerPrice,
            originalPrice: onlineCourses[0].originalPrice
          });
        }

        if (!ignore) {
          const coursesToUse = onlineCourses.length > 0 ? onlineCourses : data;
