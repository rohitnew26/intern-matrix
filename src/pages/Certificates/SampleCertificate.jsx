...
const InternshipCertificate = () => {
  ...
  const downloadPDF = async () => {
    if (!imagesLoaded) {
      alert("Please wait until all images are fully loaded!");
      return;
    }
    const element = certificateRef.current;

    // Clone the node and inline computed styles to avoid unsupported color functions
    // (e.g. oklch) that html2canvas may not parse.
    ...
    const inlineComputedStyles = (sourceEl, targetEl) => {
      try {
        ...
        for (let i = 0; i < computed.length; i++) {
          ...
          if (val) targetEl.style.setProperty(prop, val, prio);
        }
      } catch (e) {
        // ignore failures for some exotic elements
      }
    };
    ...
    for (let i = 0; i < originals.length; i++) {
      inlineComputedStyles(originals[i], clones[i]);
    }

    // Place clone off-screen so html2canvas can render it
    ...
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-4">
      ...
      <div className="flex gap-3 mb-4">
        ...
        <button
          ...
          className={`px-5 py-2 font-semibold rounded shadow ${
            imagesLoaded
      ...
      {!imagesLoaded ? (
        <div className="flex flex-col items-center justify-center h-96">
          ...
          <div className="w-24 h-24 border-8 border-blue-600 border-dashed rounded-full animate-spin"></div>
        </div>
      ) : (
                ...
                <div className="flex flex-col mt-8">
                  <div className="grid grid-cols-3 gap-8 items-end">
                    ...
                    <div className="flex flex-col items-center">
                      <div className="flex gap-7 items-center justify-center">
                        ...
                        <img src={assets.microsoftLogo} className="h-24" alt="Microsoft" />
                        <img src={assets.mocaLogo} className="h-24" alt="MOCA" />
                      </div>
                    </div>

                    <div className="text-center pr-8">
                      <img src={assets.signature} className="w-52 mx-auto mb-2" alt="Signature" />
                      <div className="border-t-4 border-blue-900 pt-3 inline-block px-8">
                        <p className="text-3xl font-semibold text-blue-900">Director</p>
                        <p className="text-3xl font-semibold text-gray-700 mt-1">Intern Matrix</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-20 bg-gradient-to-r from-slate-800 via-blue-900 to-slate-800 text-white flex justify-between items-center px-12 py-5 rounded-lg shadow-lg">
                    <div className="flex items-center gap-3">
                      ...
                      <p className="text-2xl">This is a digitally verifiable certificate</p>
                    </div>
                    <div className="text-right">
                      <p className="text-xl text-blue-200">Verify authenticity at</p>
                      <p className="text-2xl font-semibold">www.internmatrix.com/verify</p>
                    </div>
                  </div>
                </div>
              </div>
